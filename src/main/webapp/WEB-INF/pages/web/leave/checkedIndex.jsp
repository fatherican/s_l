...
//                    { "data": "colleageName"},
                    ...
                    { "data": "studentNote","orderable": false,"render": function ( data, type, full, meta ) {
                           ...
                           if(approved=='-1'){//未审批
                               ...
                               console.log("bbbbb");
                                msg ="请假原因:<br>"+(((studentNote==null)||($.trim(studentNote)==''))?'无':studentNote);
                                msg+="<br>"
                                msg+="辅导员意见:<br>"+(((instructorNote==null)||($.trim(instructorNote)==''))?'无':instructorNote);
                           }else{
                               console.log("ddd");
                               msg ="请假原因:<br>"+(((studentNote==null)||($.trim(studentNote)==''))?'无':studentNote);
                               msg+="<br>"
                               msg+="辅导员意见:<br>"+(((instructorNote==null)||($.trim(instructorNote)==''))?'无':instructorNote);
                               msg+="<br>"
                               msg+="总意见:<br>"+(((studentPipeNote==null)||($.trim(studentPipeNote)==''))?'无':studentPipeNote);
                           }
                    ...
                    { "data": "cz","orderable": false,"render": function ( data, type, full, meta ) {
                            ...
                            if(full['approved']=='0'){
                                ...
                                msg='同意';
                            }else if(full['approved']=='2'){
                                msg='待学管处审批';
                            }
                            return msg;
                        }
                    },
                    { "data": "leaveSicked","orderable": false,"render": function ( data, type, full, meta ) {
                        var msg = "";
                        if(full['approved']=='1'){//只有被同意的假条才有销假状态
                            var currentDate = new Date();
                            var leaveEndDateMillSeconds = Number(full['leaveEndDate']);
                            var currentDateMillSeconds = currentDate.getTime();
                            var duringMillSeconds = currentDateMillSeconds-leaveEndDateMillSeconds;
                            var duringMillDays = (duringMillSeconds/(24*3600000));
                            if(duringMillDays<=2){
                                msg="待销假";
                            }else{
                                if(data=='0'){
                        ...
                        return '<div class="text-center">'+msg+'</div>';
                       }
                    },
